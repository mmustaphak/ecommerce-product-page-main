...
const Cart = ({ CartQuantity, SetCartQuantity }) => {
    ...
    return (
        <div className="absolute flex flex-col w-[92%] max-w-[21.5rem] top-20 right-4 -bottom-[300%] shadow-2xl pb-4 rounded-lg bg-white desktop:top-16 desktop:max-w-[330px] desktop:max-h-[12.5rem]">
            <h2 className="pl-6 py-4 font-semibold">Cart</h2>
            <hr />
            <div className="flex flex-col justify-center w-full h-full px-4">
                {CartQuantity === 0 && <p className=" self-center font-semibold text-sm text-dark-grayish-blue">Your cart is empty.</p>}
                {
                    CartQuantity > 0 &&
                    <>
